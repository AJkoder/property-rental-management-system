...
## Note on Codex
While a hybrid Claude+Codex workflow was planned at the start of the project (Claude for architecture/business logic, Codex for boilerplate), in practice the entire committed codebase was built through Claude end-to-end, since the project's pace and the value of keeping a single consistent design/code style across sessions made it simpler not to switch tools mid-build. No Codex-generated code is present in this repository.
